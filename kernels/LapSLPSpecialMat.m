function A = LapSLPSpecialMat(t,s,side)
% special quadrature for Laplace BVP...
% close-far criteria could be modified... in line 22...
%
% Hai 12/26/20, modified from the implementation in "Solution of Stokes flow in complex nonsmooth 2D geometries via a linear-scaling high-order adaptive integral equation scheme "
...
for k=1:np
    ...
    if sum(ik)>0
        ...
        A(ik,ls+1:ls+len) = Sspecialquad(tt,ssf,s.xlo(k),s.xhi(k),side)*interpmat(ss.p,ssf.p,qntype);
    end
    ls=ls+len;
end
